...
The transformation fixes the initial values, splits the total STAT5 pool by the measured ratio and puts every estimated parameter on a log10 scale, which leaves the nine parameters of the fit.
...
Every object was built with `compile = FALSE`, so one call compiles them together into a single shared object. Compile flags are per file: the ODE and its sensitivities are built with OpenMP, the observation and transformation sources without.
...
A trust-region run finds a local optimum. A hundred fits from random starting points, sorted by value, give the waterfall plot: the lowest plateau holds the runs that reached the best optimum.
...
Maximum likelihood underestimates the standard deviations, because the residuals are shrunk by the parameters fitted to the mean. `reml()` removes that bias: each residual enters the error model weighted by one minus its own leverage, which is the stationarity condition of the restricted likelihood.

The restricted likelihood is the likelihood of error contrasts, combinations of the data without information on the mean parameters, and leaves sigma unbiased ([Patterson and Thompson (1971)](https://doi.org/10.1093/biomet/58.3.545)).
...
`remlLeverage()` reports the hat values per observable. Their sum is the rank, the effective number of mean parameters, which stays below the nominal count.
...
The band is one standard deviation from the REML estimate. The dynamic parameters move a little as well: the three correction factors differ, which reweights the observables against each other.
...
Profiles re-optimise every other parameter while one is held away from its optimum, so the interval comes from the likelihood itself and not from a quadratic approximation at the optimum ([Raue et al. (2009)](https://doi.org/10.1093/bioinformatics/btp358)).
...
Everything refers to the objective that was optimised, prior included: `stop = "value"` and `val.column = "value"` select the total rather than the data term, since only the total has its minimum at the fit.

The profiles start at that fit, because a profile is measured against the value at its origin.
