...
<!-- boehm.md is generated from boehm.Rmd. Please edit that file -->

# STAT5 dimerisation after Epo stimulation

The model of [Boehm et al. (2014)](https://doi.org/10.1021/pr5006923)
describes the response of BaF3-EpoR cells to erythropoietin: STAT5A and
STAT5B are phosphorylated, form the three dimers ApA, ApB and BpB, are
imported into the nucleus and return to the cytoplasm. The data are
relative quantities from mass spectrometry, and they ship with the
package.
...
not follow from the reactions that import them.
...
scale, which leaves the nine parameters of the fit.
...
transformation sources without.
...
prior on the dynamic parameters keeps them at finite values. The prior
stays off the `sd_*`: `reml()` below solves the stationarity condition
of the data term for those and would not see a prior on them.
...
A trust-region run finds a local optimum. A hundred fits from random
starting points, sorted by value, give the waterfall plot: the lowest
...
<img src="figures/boehm-mstrust-1.svg" alt="" width="100%" />
...
Maximum likelihood underestimates the standard deviations, because the
residuals are shrunk by the parameters fitted to the mean. `reml()`
removes that bias: each residual enters the error model weighted by one
minus its own leverage, which is the stationarity condition of the
restricted likelihood.

The restricted likelihood is the likelihood of error contrasts,
combinations of the data without information on the mean parameters, and
leaves sigma unbiased ([Patterson and Thompson
...
```
    ...
    ## ML        0.5882684      0.8202052      0.4987509
    ## REML      0.6253912      0.8441885      0.5248479

``` r
remlfit$dof     # n_g minus the summed leverage per observable
```

    ## pSTAT5A_rel pSTAT5B_rel rSTAT5A_rel 
    ##    13.67786    14.22160    14.10054
...
`remlLeverage()` reports the hat values per observable. Their sum is the
rank, the effective number of mean parameters, which stays below the
nominal count.
...
```
    ...
    ## pSTAT5A_rel pSTAT5B_rel rSTAT5A_rel 
    ##    2.365088    1.744374    1.890538

## Fit and uncertainty band

The band is one standard deviation from the REML estimate. The dynamic
parameters move a little as well: the three correction factors differ,
which reweights the observables against each other.
...
<img src="figures/boehm-bestfit-1.svg" alt="" width="100%" />

## Profile likelihood

Profiles re-optimise every other parameter while one is held away from
its optimum, so the interval comes from the likelihood itself and not
from a quadratic approximation at the optimum ([Raue et
...
Everything refers to the objective that was optimised, prior included:
`stop = "value"` and `val.column = "value"` select the total rather than
the data term, since only the total has its minimum at the fit.

The profiles start at that fit, because a profile is measured against
the value at its origin.
...
<img src="figures/boehm-profiles-1.svg" alt="" width="100%" />
...
```
    ...
    ## Epo_degradation_BaF3 Epo_degradation_BaF3 -1.5671783 -1.74093764 -1.3892256
    ## k_exp_hetero                 k_exp_hetero -4.3621242        -Inf -2.9483872
    ## k_exp_homo                     k_exp_homo -2.2011764 -2.55927401 -1.8870779
    ## k_imp_hetero                 k_imp_hetero -1.7880075 -1.91152039 -1.6575890
    ## k_imp_homo                     k_imp_homo  1.4359335  0.09591609        Inf
    ## k_phos                             k_phos  4.1978589  4.10592378  4.2971356
    ## sd_pSTAT5A_rel             sd_pSTAT5A_rel  0.5882684  0.40885933  0.7953097
    ## sd_pSTAT5B_rel             sd_pSTAT5B_rel  0.8202052  0.66400511  1.0139574
    ## sd_rSTAT5A_rel             sd_rSTAT5A_rel  0.4987509  0.34799650  0.6923901

The plot separates the two terms. For `k_imp_homo` and `k_exp_hetero`
the `data` curve stays level and only `prior` rises, slowly. Nuclear
import of the homodimers is already fast enough to be rate-limited by
phosphorylation, and the heterodimer barely returns to the cytoplasm
within the measured 240 minutes. Neither reaches the threshold inside
the searched range, so those intervals stay open on one side. The prior
keeps the fit at finite values; it does not close them.
