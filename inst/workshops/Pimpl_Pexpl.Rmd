...
p <- eqnvec(
  X = "a + b",
  Y = "a - b"
) %>% P(method = "explicit", compile = TRUE)

```

Here, the names of the vector denote the original parameters and the vector entries are expressions of the new parameters. The transformation runs compiled code, hence `compile = TRUE`. We test the result:
...
p <- eqnvec(
  X = "X + Y - 2*a",
  Y = "X - Y - 2*b"
) %>% P(method = "implicit", compile = TRUE)
```
...
p <- P(constraint, method = "implicit", compile = TRUE)
