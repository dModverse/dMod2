...
def _formula(ast):
    ...
    if ast is None:
        return "0"
    return _unwrap_unary(libsbml.formulaToL3String(_numeric_constants(ast.deepCopy())))


# MathML <pi/>, <exponentiale/> and the avogadro csymbol are values, but the
# L3 formatter prints them as identifiers, which every reader downstream takes
# for a free parameter. They are folded to their value on a copy of the tree.
_CONSTANT_VALUE = {
    libsbml.AST_CONSTANT_PI: math.pi,
    libsbml.AST_CONSTANT_E: math.e,
    libsbml.AST_NAME_AVOGADRO: 6.02214179e23,
}


def _numeric_constants(ast):
    stack = [ast]
    while stack:
        node = stack.pop()
        value = _CONSTANT_VALUE.get(node.getType())
...
# An n-ary <times/> or <plus/> with a single operand is that operand. libsbml
# renders it as a call, `times(x)`, which no downstream parser knows.
_UNARY_NARY = re.compile(r"(?<![A-Za-z0-9_.])(times|plus)\s*\(")


def _unwrap_unary(formula):
    while True:
        m = _UNARY_NARY.search(formula)
        if m is None:
            return formula
        i = m.end() - 1            # the opening parenthesis
        depth, j, comma = 0, i, False
        while j < len(formula):
            if formula[j] == "(":
                depth += 1
            elif formula[j] == ")":
                ...
                if depth == 0:
                    break
            elif formula[j] == "," and depth == 1:
                comma = True
            j += 1
        if depth != 0 or comma:
            # more than one operand, or unbalanced: leave it and stop scanning
            return formula
        formula = formula[:m.start()] + formula[i:j + 1] + formula[j + 1:]
...
def parse_sbml(sbml_file):
    ...
    parameter_names = []
    parameter_values = []
    # An <initialAssignment> on a parameter is its starting value; a parameter
    # that a RateRule drives becomes a state on the R side and needs it.
    ...
    for i in range(model.getNumParameters()):
        ...
        ia = model.getInitialAssignment(pid)
        if ia is not None and ia.isSetMath():
            parameter_assignments[pid] = _formula(ia.getMath())
