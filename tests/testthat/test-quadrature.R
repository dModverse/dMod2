...
test_that("logLhat at level L recovers the 1D-integrate truth for a one-eta prdfn", {
  ...
  for (i in seq_along(subjects)) {
    ...
    for (level in 3:5) {
      nodes <- dMod2:::makeSubjectNodes(etaHat, H_i, level)
      out   <- dMod2:::.normalEcmSubject(
        subjIdx           = i,
        psiFull           = psi,
        etaModes          = etaModes,
        omega          = om,
...
test_that("gradient at outer params matches numDeriv on the one-eta prdfn", {
  ...
  nodes_list <- lapply(seq_along(subjects), function(i)
    dMod2:::makeSubjectNodes(0, matrix(5, 1, 1), 4L))

  ofv_at <- function(mu) {
    psi_v <- psi
    psi_v["mu_pop"] <- mu
    sum(sapply(seq_along(subjects), function(i) {
      o <- dMod2:::.normalEcmSubject(i, psi_v, etaModes, om, nodes_list[[i]],
  ...
  gr_analytic <- sum(sapply(seq_along(subjects), function(i) {
    o <- dMod2:::.normalEcmSubject(i, psi, etaModes, om, nodes_list[[i]],
                              xPred = xPred, datalist = data,
                              outerActiveNames = "mu_pop",
                              mode = "with_grad")
    o$gradient["mu_pop"]
...
test_that("weight-pruning drops low-weight nodes and preserves the marginal", {
  ...
  full2 <- dMod2:::makeSubjectNodes(c(a = 0, b = 0), diag(2), level = 5L)
  tol   <- diff(range(full2$logAbsWeights)) / 2
  pr2   <- dMod2:::makeSubjectNodes(c(a = 0, b = 0), diag(2), level = 5L,
                                   pruneTol = tol)
