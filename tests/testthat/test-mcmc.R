...
test_that("smcLogSumExp / smcESS / smcSystematicResample / mcmcSmcReweight: numerics", {
  set.seed(1L)
  x <- rnorm(100L)
  expect_equal(dMod2:::smcLogSumExp(x),
               log(sum(exp(x - max(x)))) + max(x),
               tolerance = 1e-12)

  logw <- rnorm(50L)
  w    <- exp(logw - dMod2:::smcLogSumExp(logw))
  expect_equal(dMod2:::smcESS(logw), sum(w)^2 / sum(w * w), tolerance = 1e-10)

  w <- c(rep(0, 9L), 1.0)
  idx <- dMod2:::smcSystematicResample(weights = w, u = 0.1)
  expect_true(all(idx == 10L))

  N <- 50L
  logL <- rnorm(N)
  logwPrev <- rep(-log(N), N)
  rw <- dMod2:::mcmcSmcReweight(logL, logwPrev, betaOld = 0.0, betaNew = 1.0)
  ref <- (logwPrev + logL) - dMod2:::smcLogSumExp(logwPrev + logL)
...
test_that("stratified / residual / multinomial resamplers concentrate on the one heavy particle", {
  set.seed(2L)
  w <- c(rep(0, 9L), 1.0)
  for (fn in list(dMod2:::mcmcStratifiedResample,
                  dMod2:::mcmcResidualResample,
                  dMod2:::mcmcMultinomialResample)) {
    idx <- fn(weights = w)
