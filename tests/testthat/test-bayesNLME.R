...
test_that("bayesNLMEMarginal builds a likObj whose value at FOCEI MAP matches the converged OFV", {
  ...
  fit <- suppressMessages(EM(
    fx$obj, init,
    method = "focei",
    ## the assertion below is that the FOCEI mode is stationary for the exact
    ## marginal, which needs the stage-2 correction
...
test_that(".computeMetricDerivative matches FD on a decay model", {
  ...
  th <- c(A = 1.3, k = 0.4)
  ana <- dMod2:::.computeMetricDerivative(mctx, th)
