...
test_that("the batched chain kernel matches the serial path (joint + gap)", {
  ...
  batched <- run(FALSE)
  serial  <- run(TRUE)
  lineOf <- function(o) sort(vapply(o$symmetries, dMod2:::.symDirectionLine,
                                    character(1)))
...
test_that("the verification gate accepts correct and rejects wrong closed forms", {
  ...
  expect_equal(dMod2:::.symEvalModq("-(A + B)/k2", list(A = 7, B = 11, k2 = 13),
                                     qs, sd), as.integer(ref))
  expect_equal(dMod2:::.symEvalModq("-1", list(A = 7), qs, sd), as.integer(qs - 1))
  ...
  expect_true(dMod2:::.symVerifyDirection(good, 1L, znames, leafNames, point0, 1L,
                                           fakeKcall, sd))
  expect_false(dMod2:::.symVerifyDirection(bad, 1L, znames, leafNames, point0, 1L,
                                            fakeKcall, sd))
})
...
test_that("symSparsePoly recovers sparse polynomials by Ben-Or-Tiwari", {
  ...
  recover <- function(terms, bases, len, degree) {
    ...
    res <- dMod2:::symSparsePoly(as.integer(seqv), matrix(as.integer(g), ncol = nv),
  ...
  r4 <- dMod2:::symSparsePoly(as.integer(short), matrix(as.integer(g), ncol = 2),
...
test_that("symMonoResidues computes modular monomial values including Laurent", {
  ...
  E <- matrix(c(1, 0, 0,  0, 1, 0,  2, 1, 0,  1, 0, -1,  0, 0, -1),
              ncol = 3, byrow = TRUE)
  got <- dMod2:::symMonoResidues(matrix(as.integer(E), ncol = 3), as.integer(b), p)
  i5 <- modinv(5, p)
...
test_that(".symSparseEntry recovers a wide Laurent entry exactly", {
  ...
  e <- dMod2:::.symSparseEntry(1:8, sc, f, rep(1, 8), vars, 1L, fakeKcall, 0L)
...
test_that(".symGeneralRationalEntry recovers a multi-term denominator", {
  ...
  e <- dMod2:::.symGeneralRationalEntry(1:8, sc, f, point0, vars, 1L, fakeKcall, 0L)
  expect_false(is.null(e))
  expect_true(.symExprEqual(e, target))
})
...
test_that("log-coordinate gauge reconstructs a parameter-weighted scaling", {
  ...
  lg <- dMod2:::.symLogcoordGauge(1L, matrix(0L, 0L, nz), P, nz, list(ref = ref),
                                   zvals0)
  expect_equal(length(lg$anchors), 1L)

  pool <- dMod2:::.symPool()
  dir <- dMod2:::.symInterpolateDirection(
  ...
  e$vector <- dMod2:::.symLogcoordBacksub(e$vector, spy)
...
test_that("the symbolic engine agrees with the modular one and guards its limits", {
  ...
  expect_identical(dMod2:::.symDirectionLine(dmod), dMod2:::.symDirectionLine(dsym))
