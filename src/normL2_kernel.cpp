...
List normL2_kernel(
  ...
  n_threads = 1;
#endif
  // A thread with no condition still allocates its own gradient and Hessian
  // slot, so never ask for more threads than there is work.
  if (n_threads > n_cond) n_threads = std::max(1, n_cond);
  ...
  for (int c = 0; c < n_cond; ++c) {
    ...
    for (int p2 = 0; p2 < npl; ++p2) {
      ...
      for (int p1 = 0; p1 < npl; ++p1) {
        ...
        if (g1 < 0) continue;
        hess_global[(std::size_t) g1 + (std::size_t) g2 * n_par_global] +=
            hess_c[c][(std::size_t) p1 + (std::size_t) p2 * npl];
      }
    }
  }
