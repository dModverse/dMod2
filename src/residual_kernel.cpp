...
void accumulate_aloq_residual(
  ...
  for (int i = 0; i < n_obs; ++i) {
    ...
    const double wr = (pred_i - y_i) * inv_s;
    ...
    // Build dwr[k] = inv_s * dpred[k] - wr * inv_s * dsigma[k];
    //       dw0[k] = inv_s * dpred[k] - w0 * inv_s * dsigma[k] (M4BEAL or deriv2-exact)
    //       dlogs[k] = inv_s * dsigma[k].
    if (has_dsig) {
      for (int k = 0; k < n_par; ++k) {
        const double dx = dpred_i[k];
        const double ds = dsigma_i[k];
        dwr[k]   = inv_s * dx - wr * inv_s * ds;
        dlogs[k] = inv_s * ds;
        if (m4beal_aloq) dw0[k] = inv_s * dx - w0 * inv_s * ds;
      }
    } else {
      for (int k = 0; k < n_par; ++k) {
        dwr[k]   = inv_s * dpred_i[k];
        dlogs[k] = 0.0;
...
void accumulate_bloq_residual(
  ...
  for (int i = 0; i < n_obs; ++i) {
    ...
    // matching res() output for BLOQ rows
    const double wr = (pred_i - val_i) * inv_s;
    const double w0 = pred_i           * inv_s;
    ...
    if (has_dsig) {
      for (int k = 0; k < n_par; ++k) {
        ...
        const double ds = dsigma_i[k];
        dwr[k] = inv_s * dx - wr * inv_s * ds;
        dw0[k] = inv_s * dx - w0 * inv_s * ds;
      }
    } else {
      for (int k = 0; k < n_par; ++k) {
        const double dx = inv_s * dpred_i[k];
        dwr[k] = dx;
        dw0[k] = dx;
      }
    }
